...
def liar_V(value: int, random) -> int:
    if random.random() > 0.5 and value % 2 == 0:
        value //= 2
    return value

class Rule1Ldouble(AbstractClueRule):
    name = ["1L^", "L^", "两倍值误差", "Liar^"]
    doc = "线索的实际值为线索值的2倍或不变"
    ...
    def fill(self, board: 'AbstractBoard') -> 'AbstractBoard':
        ...
        for pos, _ in board("N"):
            ...
            board.set_value(pos, Value1Ldouble(pos, count=value))
            logger.debug(f"Set {pos} to 1Ldouble[{value}]")
        return board


class Value1Ldouble(AbstractClueValue):
    ...
    @classmethod
    def type(cls) -> bytes:
        return Rule1Ldouble.name[0].encode("ascii")
    ...
    def create_constraints(self, board: 'AbstractBoard', switch):
        ...
        if neighbor_vars:
            ...
            b1 = model.NewBoolVar("sum_eq_count_same")
            b2 = model.NewBoolVar("sum_eq_count_double")
