    ...
    def fill(self, board: AbstractBoard) -> AbstractBoard:
        ...
        for key in board.get_board_keys():
            for pos, _ in board("N", key=key):
                ...
                board.set_value(pos, ValueBIN(pos, int(values[get_random().randint(0, 1)].__repr__()), self.rule, None))
        return board
...
class ValueBIN(AbstractClueValue):
    def __init__(self, pos: 'AbstractPosition', value: int = 0, rule: tuple[str, str] = ("", ""), code: bytes | None = None):
        ...
        if not code:
            ...
            self.rule = (code[1:split_index].decode("ascii"), code[split_index + 1:].decode("ascii"))
    ...
    def create_constraints(self, board: AbstractBoard, switch):
        ...
        clue1 = self.get_clue(self.value, self.rule[0])
        clue2 = self.get_clue(self.value, self.rule[1])
