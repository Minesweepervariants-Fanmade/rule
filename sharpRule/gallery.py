...
class Rule1B(AbstractMinesRule):
    ...
    def create_constraints(self, board: 'AbstractBoard', switch):
        ...
        for key in board.get_interactive_keys():
            ...
            row_positions = board.get_col_pos(boundary_pos)
            row_sums = [
...
class Rule1B_(AbstractMinesRule):
    id = "1B|"
    aliases = ("B|",)
    name = "Column Balance"
    name.zh_CN = "列平衡"
    doc = "Each column has the same number of mines"
    doc.zh_CN = "每列雷数相等"
    author = ("", 0)
    tags = ["Untagged"]
    creation_time = ""
    ...
    def create_constraints(self, board: 'AbstractBoard', switch):
        ...
        for key in board.get_interactive_keys():
            ...
            for i in range(1, len(col_sums)):
                model.Add(col_sums[i] == col_sums[0]).OnlyEnforceIf(s1)